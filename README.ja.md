...
```
opensystem> ポモドーロタイマーを作って
  Classifying intent... CreateApp
  → Generating AppSpec from prompt...
  → App: "ポモドーロタイマー" — 25分集中タイマー、開始/停止ボタン付き
...
opensystem> ポモドーロを実行して
  → Running: ポモドーロタイマー (v0.1.0)
  → Executing WASM sandbox...
  ✓ App output:
    ポモドーロタイマーを開始しました。25分間集中してください。
```
...
| WASM サンドボックス実行 | ✅ 動作中 | wasmtime 42 / WASIp1、`MemoryOutputPipe` 出力キャプチャ |
...
| GPUアクセラレーションレンダリング | 🔜 v2.1 | Bevy + wgpu（ECSツリー接続待ち）|
| WASM実行時間制限 | 🔜 v2.1 | epochインタラプトCPUバジェット |
...
ユーザーの意図
    ↓
os-agent が分類 → CreateApp
    ↓
LLM が並行生成：
  ┌─────────────────┐    ┌──────────────────────────┐
  │  Rust/WASMコード │    │  UIDL JSON（ウィジェットツリー）│
  │  cargo check    │    │  検証後パッケージに書き込み │
...
| `app-store` | Ed25519署名 `.osp` レジストリ、HTTP API、`osctl` CLI | — |

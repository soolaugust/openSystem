...
pub async fn run_setup_wizard() -> Result<()> {
    println!("╔══════════════════════════════════════════════╗");
    println!("║         openSystem First Boot Setup Wizard         ║");
    println!("╚══════════════════════════════════════════════╝");
    println!();
    println!("Welcome to openSystem — The OS that assumes you have AI.");
    ...
    println!();
    println!("✓ Setup complete! openSystem is ready to use.");
    println!("  Type your first intent at the opensystem> prompt.");
    println!();
...
fn test_connectivity() -> Result<()> {
    ...
    match output {
        ...
        _ => {
            ...
            println!("  openSystem requires internet for AI inference.");
...
async fn configure_ai_endpoint() -> Result<(ApiEndpointConfig, Option<ApiEndpointConfig>)> {
    ...
    match test_api(&primary).await {
        ...
        Err(e) => {
            ...
            println!("  Saving anyway. You can re-run setup with: opensystem-setup");
        }
    }
...
async fn test_api(config: &ApiEndpointConfig) -> Result<()> {
    ...
    let request = serde_json::json!({
        "model": config.model,
        "messages": [{"role": "user", "content": "Reply with: OPENSYSTEM_TEST_OK"}],
        "max_tokens": 16,
        "temperature": 0.0
    });
...
fn save_config(primary: &ApiEndpointConfig, fallback: Option<&ApiEndpointConfig>) -> Result<()> {
    ...
    let mut config = format!(
        r#"# openSystem Model Configuration
# Generated by setup wizard — do not edit manually
# Re-run setup: opensystem-setup
